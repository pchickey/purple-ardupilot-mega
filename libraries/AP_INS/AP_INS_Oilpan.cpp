...
const uint8_t AP_INS_Oilpan::_sensors[6] = { 1, 2, 0, 4, 5, 6 };

// ADC result sign adjustment for sensors.
const  int8_t AP_INS_Oilpan::_sensor_signs[6] = { 1, -1, -1, 1, -1 , -1 };

// ADC channel reading the gyro temperature
const uint8_t AP_INS_Oilpan::_gyro_temp_ch = 3;

// Maximum possible value returned by an offset-corrected sensor channel
const float AP_INS_Oilpan::_adc_constraint = 900;
...
const float AP_INS_Oilpan::_gravity = 423.8;

///< would like to use _gravity here, but cannot
const float AP_INS_Oilpan::_accel_scale = 9.80665 / 423.8; 

#define ToRad(x) (x*0.01745329252)  // *pi/180
// IDG500 Sensitivity (from datasheet) => 2.0mV/degree/s,
// 0.8mV/ADC step => 0.8/3.33 = 0.4
// Tested values : 0.4026, ?, 0.4192
const float AP_INS_Oilpan::_gyro_gain_x = ToRad(0.4);
const float AP_INS_Oilpan::_gyro_gain_y = ToRad(0.41);
const float AP_INS_Oilpan::_gyro_gain_z = ToRad(0.41);

/* ------ Public functions -------------------------------------------*/

AP_INS_Oilpan::AP_INS_Oilpan( AP_ADC * adc ) :
  _adc(adc)
{
  _gyro.x = 0;
  _gyro.y = 0;
...
float AP_INS_Oilpan::gx() { return _gyro.x; }
float AP_INS_Oilpan::gy() { return _gyro.y; }
float AP_INS_Oilpan::gz() { return _gyro.z; }

void AP_INS_Oilpan::get_gyros( float * g )
{
  g[0] = _gyro.x;
  g[1] = _gyro.y;
  g[2] = _gyro.z;
...
float AP_INS_Oilpan::ax() { return _accel.x; }
float AP_INS_Oilpan::ay() { return _accel.y; }
float AP_INS_Oilpan::az() { return _accel.z; }

void AP_INS_Oilpan::get_accels( float * a )
{
  a[0] = _accel.x;
  a[1] = _accel.y;
  a[2] = _accel.z;
...
void AP_INS_Oilpan::get_sensors( float * sensors )
{
  sensors[0] = _gyro.x;
  sensors[1] = _gyro.y;
  sensors[2] = _gyro.z;
...
float AP_INS_Oilpan::temperature() { return _temp; }

uint32_t AP_INS_Oilpan::sample_time() { return _sample_time; }

/* ------ Private functions -------------------------------------------*/

float AP_INS_Oilpan::_gyro_apply_std_offset( uint16_t adc_value )
{
  /* Magic number from AP_ADC_Oilpan.h */
  return ((float) adc_value ) - 1658.0f;
}

float AP_INS_Oilpan::_accel_apply_std_offset( uint16_t adc_value )
{
  /* Magic number from AP_ADC_Oilpan.h */
